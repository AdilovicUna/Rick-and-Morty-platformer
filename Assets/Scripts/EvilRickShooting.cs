...
public class EvilRickShooting : MonoBehaviour
{
    ...
    void Update()
    {
        if (canShoot)
        {
            ...
            if (cooldown < timer)
            {
                ...
                int dir = GetComponent<EvilRickMovement>().getDirection();
                ...
                bulletInstance.GetComponent<Bullet>().setKillTag("MainRick");
                bulletInstance.GetComponent<Bullet>().setIgnoreTag("EvilRick");
            }
        }
        else
        {
